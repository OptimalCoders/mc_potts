...
namespace mc_potts {
    ...
    //------------------------single-spin update------------------------//
        void update_spin_() {
            ...
            spin_t direction = rngdir();
            direction = direction * 2 - 1;
            
            spin_t temp = system_.get(i, j, k) + direction;
            
            //return if the new state isn't a valid state
            if(abs(temp) >= S or abs(temp) < 0) {
                return;
            }
            
            // acceptance step
            prob_t p = exp(physical_const / T_ * direction * (system_.get_nn(i, j, k) - 8 * (S - 1) / 2.));
            prob_t r = rngprob();
            if(p > r) {
