...
#include <beta.hpp>
#include <../install/install_full.hpp>
...
namespace mc_potts {
    namespace install {
        ...
        struct size_helper<S, 0, false> {
            static constexpr int size = sizes[0];
        };

        template<int S>
        struct size_helper<S, num_sizes - 1, true> {
            static constexpr int size = sizes[num_sizes - 1];
        };

        template<int S>
        struct size_finder {
            static constexpr int size = size_helper<S, num_sizes - 1, S >= sizes[num_sizes - 1] >::size; 
        };
